...
void ordenaQuick(int quantidade, int situacao, bool imprimir) {
    const char *entrada;
    switch (situacao) {
        case 1: entrada = "Ascendente.bin"; break;
        case 2: entrada = "Descendente.bin"; break;
        case 3: entrada = "PROVAO.bin"; break;
        default:
    ...
    if (!copia_n_registros(entrada, work, quantidade)) return;
    ...
    if (!ArqLi || !ArqEi || !ArqLEs) {
        ...
        if (ArqLi) fclose(ArqLi);
        if (ArqEi) fclose(ArqEi);
        if (ArqLEs) fclose(ArqLEs);
        return;
    }
...
void imprime_primeiros(const char *nomeArquivo, int qtd) {
    ...
    int lim = (qtd < 5) ? qtd : 5;
    for (int i = 0; i < lim; i++) {
        if (fread(&a, sizeof(Aluno), 1, f) != 1) break;
...
int copia_n_registros(const char *src, const char *dst, int n) {
    ...
    if (!in) { fprintf(stderr, "Erro: não consegui abrir %s\n", src); return 0; }
    FILE *out = fopen(dst, "wb");
    if (!out) { fprintf(stderr, "Erro: não consegui criar %s\n", dst); fclose(in); return 0; }

    Aluno a;
    for (int i = 0; i < n; i++) {
        if (fread(&a, sizeof(Aluno), 1, in) != 1) break;
