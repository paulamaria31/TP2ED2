...
void faseGeracaoBlocos(FILE *arq, int *numBlocos, int quantidade)
{
    ...
    for (int i = elementosNoHeap / 2 - 1; i >= 0; i--)
        refazerHeap2F(heap, i, elementosNoHeap);
    ...
    while (elementosNoHeap > 0)
    {
        ...
        if (elementosNoHeap > 0 && marcados == elementosNoHeap)
        {
            ...
            for (int i = 0; i < elementosNoHeap; i++)
                heap[i].marcado = 0;
        ...
        if (elementosNoHeap > 0)
            refazerHeap2F(heap, 0, elementosNoHeap);
    }
